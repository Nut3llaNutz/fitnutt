...
const CHAPTERS: Chapter[] = [
  {
    ...
    message: "Welcome to FitNutt — your personal shred coach. I literally never stop pumping iron. Let me show you around so you know exactly where everything lives.",
  ...
  },
  {
    ...
    message: "These are your macro rings — Calories, Protein, Carbs, and Fats. Tap any ring to see a breakdown of exactly which foods are contributing.",
  },
  {
    route: "/",
    target: '[data-tour="meal-log"]',
    title: "The Diary 🍽️",
    message: "Your food diary. Breakfast, lunch, dinner, snacks — every meal logged here feeds into those rings in real time.",
  },
  {
    route: "/",
    target: '[data-tour="streak-btn"]',
    title: "Your Streak 🔥",
    message: "See that flame? That's your consistency streak. Log food or tick off supplements every day to keep it burning. Let it die and Nut3lla gets angry.",
  },
  {
    route: "/foods",
    target: '[data-tour="food-search"]',
    title: "The Food Library 🥗",
    message: "Search through 200+ preset foods — Chicken Tikka to Maggi. Add your own custom foods and track by any unit.",
  },
  {
    route: "/foods",
    target: '[data-tour="veg-filter"]',
    title: "Veg Mode 🌱",
    message: "Toggle this to filter only vegetarian options. FitNutt respects the diet — veg, non-veg, whatever you're about.",
  },
  {
    route: "/schedule",
    target: '[data-tour="schedule-workouts"]',
    title: "The Playbook 📋",
    message: "Plan your weekly workout splits and meal strategy. Structure is what separates the serious from the mediocre.",
  },
  {
    route: "/pump-rank",
    target: '[data-tour="rank-hero"]',
    title: "Pump Rank 🏆",
    message: "Every meal logged, every supplement ticked, every calorie target hit — earns XP. Level up from Gym Novice to GOD OF IRON.",
  },
  {
    // Final protocol form — no spotlight
    route: "/",
    title: "Your Protocol ⚡",
    message: "Tour done. Give me your stats and I'll calculate your exact calorie and macro targets in seconds.",
    nextLabel: "LET'S GO",
  },
];
    ...
    <div className="fixed bottom-[6.5rem] right-3 z-[180] animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="relative flex items-end gap-2">
        ...
        <div className="relative h-14 w-14 shrink-0 [filter:drop-shadow(0_8px_16px_rgba(0,0,0,0.3))] pointer-events-none">
          <img src="/fitnutt-logo.png" alt="" className="absolute inset-0 h-14 w-14 animate-logo-pump-up" />
          <img src="/fitnutt-logo-down.png" alt="" className="absolute inset-0 h-14 w-14 animate-logo-pump-down" />
        </div>
...
export const TutorialOverlay = () => {
  ...
  const spotlightRect = useSpotlight(isLast ? undefined : currentChapter?.target);
  ...
  const saveProtocolToLocalStorage = (targets: ReturnType<typeof calculateMacros>) => {
    const existing = JSON.parse(localStorage.getItem("fitnutt_guest_settings") || "{}");
    localStorage.setItem("fitnutt_guest_settings", JSON.stringify({
      ...
      gender, weight_kg: parseFloat(weightKg),
  ...
  const handleContinueExploring = async () => {
    ...
    try {
      const targets = calculateMacros({
        gender, weight_kg: parseFloat(weightKg), height_cm: parseFloat(heightCm),
        age: parseInt(age), activity_level: parseFloat(activity), goal,
      });
      if (user) {
        // @ts-ignore
        ...
        saveProtocolToLocalStorage(targets);
      }
      toast({ title: "Protocol locked in! 🔒", description: "Macro targets set." });
      completeTutorial();
    } catch (err: any) {
  ...
  return (
    <div id="tutorial-overlay" className="fixed inset-0 z-[150] flex flex-col overflow-hidden pointer-events-none">
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-auto">
        <defs>
          ...
          <mask id="tut-mask">
            ...
            {spotlightRect && (
              <rect
                x={spotlightRect.left - 10} y={spotlightRect.top - 10}
                width={spotlightRect.width + 20} height={spotlightRect.height + 20}
                rx="14" fill="black"
              />
            )}
          </mask>
        </defs>
        <rect width="100%" height="100%" fill="rgba(0,0,0,0.78)" mask="url(#tut-mask)" />
        ...
        {spotlightRect && (
          <rect
            ...
            width={spotlightRect.width + 20} height={spotlightRect.height + 20}
            rx="14" fill="none"
            stroke="hsl(var(--primary))" strokeWidth="2.5" opacity="0.8"
            className="animate-pulse" style={{ filter: "url(#tut-glow)" }}
          />
        )}
      </svg>
        ...
        {CHAPTERS.map((_, i) => (
          <div
            ...
            className={`h-1.5 rounded-full transition-all duration-300 ${
              i === chapter ? "w-5 bg-primary" : i < chapter ? "w-1.5 bg-primary/50" : "w-1.5 bg-white/20"
            }`}
          />
        ))}
      </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    {
                      label: "Goal", value: goal, onChange: setGoal,
                      options: [{ v: "bulk", l: "Bulk" }, { v: "cut", l: "Cut" }, { v: "maintain", l: "Maintain" }]
                    },
                  ].map(({ label, value, onChange, options }) => (
                    <div key={label} className="space-y-1">
                      <Label className="text-[9px] uppercase font-black text-muted-foreground/70">{label}</Label>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { label: "Weight", value: weightKg, onChange: setWeightKg, unit: "kg" },
                    { label: "Height", value: heightCm, onChange: setHeightCm, unit: "cm" },
                    { label: "Age",    value: age,      onChange: setAge,      unit: "yr" },
                ...
                <div className="space-y-1">
                  <Label className="text-[9px] uppercase font-black text-muted-foreground/70">Activity Level</Label>
                ...
                <div className="space-y-2 pt-1">
                  ...
                  >
                    {isSubmitting ? <Loader2 className="h-5 w-5 animate-spin" /> : "SIGN UP & LOCK IT IN 🔒"}
                  </Button>
                  <Button
                    onClick={handleContinueExploring}
                    disabled={isSubmitting}
                    variant="outline"
                    className="w-full h-9 font-bold text-xs tracking-widest rounded-xl border-primary/20"
                  >
...
// ─── Legacy export ────────────────────────────────────────────────────────────
export const TutorialFlow = ({ onComplete }: { onComplete: () => void }) => <TutorialOverlay />;

  ...
  const filtered = useMemo(() => {
    return foods.filter((f) => {
      ...
      if (activeCategory !== "All")
        matchesCategory = f.category === activeCategory;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-1">
          <Button
            ...
            disabled={
              currentDate <= (user?.created_at?.split("T")[0] || getTodayStr())
            }
          >
            <ChevronLeft className="h-5 w-5" />
          </Button>
          <h1
            className="text-xl font-bold text-foreground min-w-[110px] text-center"
      ...
      <div className="flex items-center gap-3 px-1">
        <div className="relative flex-1" data-tour="food-search">
        ...
        <div className="flex items-center gap-2 bg-card px-3 py-2 rounded-xl ring-1 ring-muted" data-tour="veg-filter">
          <span
            className={`text-[9px] font-black uppercase tracking-widest ${isVegOnly ? "text-green-500" : "text-red-500"}`}
          >
            ...
            >
              <div className="flex justify-between items-center mb-2.5">
                <div className="flex items-center gap-2 min-w-0">
                  <div
                    className={`h-2 w-2 rounded-full flex-shrink-0 ${f.is_veg ? "bg-green-500" : "bg-red-500"}`}
                  />
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col items-center gap-1.5 pt-4">
                <span
                  className={`text-[8px] font-black uppercase ${form.is_veg ? "text-green-500" : "text-red-500"}`}
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <div className="flex gap-1">
                  <Input
                    type="number"
                    value={form.serving_size === 0 ? "" : form.serving_size}
                    onChange={(e) =>
                      setForm({
                        ...form,
                        serving_size:
        ...
        <DialogContent className="w-[calc(100%-2.5rem)] max-w-[360px] rounded-3xl p-6">
          ...
          <div className="space-y-6">
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-3 bg-muted/30 p-4 rounded-xl focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                <input
                  type="number"
                  value={logForm.quantity === "0" ? "" : logForm.quantity}
                  onChange={(e) =>
                    setLogForm({ ...logForm, quantity: e.target.value })
                  }
                  className="text-3xl font-black bg-transparent border-none p-0 h-auto focus:outline-none focus:ring-0 min-w-0 w-full [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                  autoFocus
                />
                <div className="flex items-center gap-2 shrink-0 border-l border-muted-foreground/20 pl-3">
                  <span className="text-muted-foreground font-black text-xl">
                    ×
                  </span>
                  <span className="font-black text-primary text-xl">
                    {loggingFood?.serving_size && loggingFood.serving_size > 1
                      ? loggingFood.serving_size
                      : ""}
                    {loggingFood?.serving_unit}
                  </span>
                </div>
              </div>
            </div>

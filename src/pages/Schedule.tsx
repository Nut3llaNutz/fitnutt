...
const Schedule = () => {
  ...
  const handleReset = () => {
    if (
      window.confirm(
        "Reset to the default schedule? All your changes will be lost.",
      )
    ) {
      resetToDefault();
      setEditMode(false);
  ...
  return (
    <div className="space-y-6" data-tour="schedule-workouts">
      ...
      <div className="flex gap-1 w-full px-1 py-1">
        {days.map((d) => (
          ...
          >
            ...
            {d === currentDayName && (
              <div
                className={`absolute -top-1 -right-1 w-2 h-2 rounded-full bg-primary border-2 border-background ${selected === d ? "hidden" : "block"}`}
              />
            )}
          </button>
        ))}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            {editMode ? (
              ...
              <h2 className="text-xl font-black italic text-card-foreground tracking-tight">
                {day.title}
              </h2>
            )}
          </div>
                ...
                {editMode ? (
                  ...
                  <>
                    ...
                    >
                      <Check
                        strokeWidth={4}
                        className={`h-4 w-4 transition-transform duration-500 ${isCompleted ? "scale-100" : "scale-0"}`}
                      />
                    </button>
                    <div className="flex-1">
                      <p
                        className={`font-bold text-sm tracking-tight ${isCompleted ? "line-through text-muted-foreground" : "text-card-foreground"}`}
                      >
                        {ex.name}
                      </p>
